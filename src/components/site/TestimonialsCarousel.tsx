      ...
      <div className="max-w-7xl mx-auto px-4 flex">
        ...
        <div className="overflow-x-hidden w-full relative ">
          <Swiper
            ...
            slidesPerView={1}
            spaceBetween={32}
            style={{ paddingBottom: '2rem', direction: 'rtl', color: 'black' }}
          >
            {/* מעבר חלק יותר */}
            {testimonials.map((t, i) => (
              <SwiperSlide key={i}>
                <div className="flex flex-col items-center justify-between bg-transparent min-w-[620px] max-w-[620px] mx-auto px-4">
                  <p
                  ...
                  <div className="flex flex-col items-right gap-2 px-6 ml-106" style={{ fontFamily: 'Tahoma, Geneva, sans-serif', color: 'black' }}>
                    {t.logo && <img src={t.logo} alt="לוגו" className="w-14 h-14 " />}
                    {t.name && <span className="font-bold text-black text-base mt-2 text-xl">{t.name}</span>}
                    {t.contact && <span className="text-gray-600 text-sm text-xl">{t.contact}</span>}

                    {t.title && <span className="text-gray-600 text-sm ">{t.title}</span>}
                  </div>
                </div>
              </SwiperSlide>
            ))}
